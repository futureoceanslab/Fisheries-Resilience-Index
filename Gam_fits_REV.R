...
##################	 	ECOLOGICAL DIMENSION 			#################
	g0e=gam(RI~sp+s(LAT,by=sp),family = "quasibinomial", data=Data[[1]])
	summary(g0e) ## Significant effect of latitude for cod.
	gambtab(g0e)


	g1e=gam(RI~sp+s(LAT,bs="ts", by=sp),family = "quasibinomial", data=Data[[1]])
...
Formula:
RI ~ sp + s(LAT, bs = "ts", by = sp)

Parametric coefficients:
            Estimate Std. Error t value    Pr(>|t|)    
(Intercept) -0.24843    0.02811  -8.837 0.000002536 ***
spHake       0.49637    0.04595  10.803 0.000000345 ***
---
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

Approximate significance of smooth terms:
                 edf Ref.df     F p-value  
s(LAT):spCod  8.6166      9 4.024  0.0133 *
s(LAT):spHake 0.4012      6 0.111  0.2233  
---
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

R-sq.(adj) =  0.872   Deviance explained = 93.3%
GCV = 0.0050623  Scale est. = 0.0025271  n = 22

########################################################################	

### marginal model for cod 
	data=Data[[1]]
	data_sp=split(data, data$sp)
	ge_cod=gam(RI~s(LAT,bs="ts"),family = "quasibinomial", data=data_sp$Cod)
	anova(ge_cod) # p-value = 0.131, non-significant effect fo latitude.
